...
@Component({
  selector: 'app-warna',
  templateUrl: './warna.page.html',
  styleUrls: ['./warna.page.scss'],
  animations: [
    trigger(
      'inOutAnimation',
      [
...
export class WarnaPage implements OnInit {
  ...
  generateQuestions(data:any){
    ...
    for(let i = 0; i < data.length; i++) {
      let newData = {
        ...
        warna: this.base_url+"tiwi_warna_"+(data[i]+1)+".png",
        opsi: [
          this.base_url+"tiwi_warna_"+(data[i]+1)+"_a.png",
          this.base_url+"tiwi_warna_"+(data[i]+1)+"_b.png",
          this.base_url+"tiwi_warna_"+(data[i]+1)+"_c.png"
        ]
      }
      ...
      buttons: [
        {
          ...
          handler: (data) => {
            this.diff = data
            this.base_url = "../../../assets/quiz/"+this.diff+"/warna/"
            this.gameStarting()
          }
        }
      ]
