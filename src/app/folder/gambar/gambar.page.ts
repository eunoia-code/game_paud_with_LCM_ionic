...
@Component({
  selector: 'app-gambar',
  templateUrl: './gambar.page.html',
  styleUrls: ['./gambar.page.scss'],
  animations: [
    trigger(
      'inOutAnimation',
      [
...
export class GambarPage implements OnInit {
  ...
  generateQuestions(data:any){
    ...
    for(let i = 0; i < data.length; i++) {
      let newData = {
        ...
        gambar: this.base_url+"tiwi_gambar_"+(data[i]+1)+".png",
        opsi: [
          this.base_url+"tiwi_gambar_"+(data[i]+1)+"_a.png",
          this.base_url+"tiwi_gambar_"+(data[i]+1)+"_b.png",
          this.base_url+"tiwi_gambar_"+(data[i]+1)+"_c.png"
        ]
      }
      ...
      buttons: [
        {
          ...
          handler: (data) => {
            this.diff = data
            this.base_url = "../../../assets/quiz/"+this.diff+"/gambar/"
            this.gameStarting()
          }
        }
      ]
  ...
  boxAnswer(aid:any){
    if(this.diff=="easy")
      if([12, 13, 14, 15].includes(aid))
        return "answer-box"
      else
        return "answer-box-large"
    else
      if([0, 1, 2, 3, 4, 6, 9, 15, 17].includes(aid))
        return "answer-box-large"
      else
        return "answer-box"
  }
