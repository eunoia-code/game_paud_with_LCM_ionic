...
@Component({
  selector: 'app-suara',
  templateUrl: './suara.page.html',
  styleUrls: ['./suara.page.scss'],
  animations: [
    trigger(
      'inOutAnimation',
      [
...
export class SuaraPage implements OnInit {
  @ViewChildren("myAudio") myAudio: QueryList<any>;
  @ViewChildren("myAudioOpsi") myAudioOpsi: QueryList<any>;
  playerName: string = ""
  questions: any = []
  base_url: string = ''
  q_opsi: any = ['A', 'B', 'C']
  ...
  generateQuestions(data: any) {
    ...
    for (let i = 0; i < data.length; i++) {
      let newData = {
        id: data[i],
        audio: this.base_url + "audio/" + (data[i] + 1) + ".ogg",
        suara: this.base_url + "0.png",
        opsi: [
          this.base_url + "tiwi_suara_" + (data[i] + 1) + "_a.png",
          this.base_url + "tiwi_suara_" + (data[i] + 1) + "_b.png",
          this.base_url + "tiwi_suara_" + (data[i] + 1) + "_c.png"
        ],
        zxc: [
          this.base_url + "0_a.png",
          this.base_url + "0_b.png",
          this.base_url + "0_c.png"
        ],
        qwe:[
          this.base_url + "opsi/" + (data[i] + 1) + "_a.ogg",
          this.base_url + "opsi/" + (data[i] + 1) + "_b.ogg",
          this.base_url + "opsi/" + (data[i] + 1) + "_c.mp3"
        ]
  ...
  playAudioOpsi(id: number, o: string) {
    this.myAudioOpsi.forEach(snap => {
      if (id+"_"+o == (snap.nativeElement.id)) {
        snap.nativeElement.play()
      } else {
        snap.nativeElement.pause()
        snap.nativeElement.currentTime = 0;
      ...
      buttons: [
        {
          ...
          handler: (data) => {
            this.diff = data
            this.base_url = "../../../assets/quiz/" + this.diff + "/suara/"
            this.gameStarting()
          }
        }
      ]
  ...
  checkFileType(aid:any){
    if(this.diff=="easy")
      if([4, 5, 6, 7, 11, 12, 13].includes(aid))
        return true
      else
        return false
  }
