...
@Component({
  selector: 'app-angka',
  templateUrl: './angka.page.html',
  styleUrls: ['./angka.page.scss'],
  animations: [
    trigger(
      'inOutAnimation',
      [
...
export class AngkaPage implements OnInit {
  ...
  generateQuestions(data:any){
    ...
    for(let i = 0; i < data.length; i++) {
      let newData = {
        ...
        gambar: this.base_url+"tiwi_angka_"+(data[i]+1)+".png",
        opsi: [
          this.base_url+"tiwi_angka_"+(data[i]+1)+"_a.png",
          this.base_url+"tiwi_angka_"+(data[i]+1)+"_b.png",
          this.base_url+"tiwi_angka_"+(data[i]+1)+"_c.png"
        ]
      }
      ...
      buttons: [
        {
          ...
          handler: (data) => {
            this.diff = data
            this.base_url = "../../../assets/quiz/"+this.diff+"/angka/"
            this.gameStarting()
          }
        }
      ]
